...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
   if InStr(FF.Name,"я") then
     FSO.CopyFile FDir&FF.Name, FDir&Mid(FF.Name,InStr(FF.Name,"я")+2,InStr(FF.Name," ")-InStr(FF.Name,")")-1) & "." & InExt
...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Sum(Val(SUMMA)) AS Summ FROM ["&TableName&"]"
  St = dbfPrice.Fields("NOMDOK") & Sep & dbfPrice.Fields("DATADOK") & Sep & dbfSum("Summ")
  St=Convert866to1251(St) 
  DeviceOut.WriteLine St
  DeviceOut.WriteLine "[Body]"
  
    Do While Not dbfPrice.Eof
	  St = dbfPrice("KODTOV") & Sep & dbfPrice.Fields("NAMETOV") & Sep & dbfPrice.Fields("PROIZ") & Sep & dbfPrice.Fields("STRANA") & Sep &_
  	    dbfPrice.Fields("KOL") & Sep & dbfPrice.Fields("CENA") & Sep & Sep & dbfPrice.Fields("CENA") & Sep &_
		Sep & Sep & Sep & dbfPrice.Fields("GTD") & Sep & "^" &_
		dbfPrice.Fields("SERT") & "^" & dbfPrice.Fields("DNSERT") & " " & dbfPrice.Fields("DKSERT") & Sep &_
		Sep & Sep & dbfPrice.Fields("GODENDO") & Sep & Sep & Sep & Sep & Sep & dbfPrice.Fields("SUMMA") & Sep & ""
          St=Convert866to1251(St)
          DeviceOut.WriteLine(St)
	  dbfPrice.MoveNext
