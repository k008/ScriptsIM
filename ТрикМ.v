...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  St=Trim(dbfPrice.Fields("Num_DOC")) & Sep & dbfPrice.Fields("DATE_DOC") & Sep
  'St=Convert866to1251(St)
  DeviceOut.WriteLine(St)
  DeviceOut.WriteLine "[Body]"
    Do While Not dbfPrice.Eof
      St=dbfPrice.Fields("KOD")  & Sep & dbfPrice.Fields("NAME") & Sep & dbfPrice.Fields("PROIZV") & Sep & dbfPrice.Fields("COUNTRY") & Sep & dbfPrice.Fields("KOLVO") & Sep & dbfPrice.Fields("SUMMA")/dbfPrice.Fields("KOLVO") & Sep & dbfPrice.Fields("CENAPROIZ") & Sep & dbfPrice.Fields("SUMMA")/dbfPrice.Fields("KOLVO")-dbfPrice.Fields("SUMMA")/dbfPrice.Fields("KOLVO")/(dbfPrice.Fields("NDSPROIZV")+100)*dbfPrice.Fields("NDSPROIZV") & Sep & Sep & Sep & Sep & dbfPrice.Fields("N_DECLAR") & Sep & dbfPrice.Fields("SERTIF") & "^" & dbfPrice.Fields("DATAEND") & "^" & dbfPrice.Fields("DATAREGSE") & Sep & dbfPrice.Fields("SERII") & Sep & Sep & dbfPrice.Fields("SROK") & Sep & Sep & Sep & Sep & Sep & dbfPrice.Fields("SUMMA") & Sep & dbfPrice.Fields("GNVLN")
