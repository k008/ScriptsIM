...
Dim TableName,dbfPrice,dbfSum,dbfConn,ZV,k
 ...
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Count(NAME_TV) AS Summ FROM "&TableName
    Do While Not dbfPrice.Eof
	  ...
	  k = k+1
	  if dbfPrice.Fields("GV") = True then
	    ZV = 1
	  else
	    ZV = 0
	  end if
      St=dbfPrice.Fields("COD_TV") & Sep & dbfPrice.Fields("NAME_TV") & Sep & dbfPrice.Fields("IZGOT") & Sep & ReplaceStr(dbfPrice.Fields("STRANA"),Sep,",") & Sep &_
	    dbfPrice.Fields("KOL") & Sep & dbfPrice.Fields("SUMMA")/dbfPrice.Fields("KOL") & Sep & dbfPrice.Fields("CENA_Z") & Sep & dbfPrice.Fields("SUMMA")/dbfPrice.Fields("KOL")-dbfPrice.Fields("SUMMA")/dbfPrice.Fields("KOL")*dbfPrice.Fields("NDS")/(dbfPrice.Fields("NDS")+100) & Sep &_
		Sep & dbfPrice.Fields("NAC") & Sep & Sep & ReplaceStr(dbfPrice.Fields("GTD"),Sep,",") & Sep & "^" &_
		ReplaceStr(dbfPrice.Fields("PROT"),Sep,",") & Sep & dbfPrice.Fields("SER") & Sep & Sep & dbfPrice.Fields("DATA_GOOD") & Sep &_
		Sep & dbfPrice.Fields("DATA_REG") & Sep & dbfPrice.Fields("CENA_REE") & Sep &_
