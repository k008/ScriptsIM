...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Sum(SUMMA) AS Summ FROM "&TableName 'запрос на расчет суммы по накладно
  St=dbfPrice.Fields("NUM_DOC") & Sep & dbfPrice.Fields("DATE_DOC") & Sep & dbfSum("Summ")
  ...
  DeviceOut.WriteLine "[Body]"
    ...
    Do While Not dbfPrice.Eof
      St=dbfPrice.Fields("KOD") & Sep & dbfPrice.Fields("NAME") & Sep & dbfPrice.Fields("PROIZV") & Sep & dbfPrice.Fields("COUNTRY") & Sep &_
	    dbfPrice.Fields("KOLVO") & Sep & dbfPrice.Fields("CENASNDS") & Sep & Sep & dbfPrice.Fields("CENABNDS") & Sep &_
		Sep & Sep & Sep & dbfPrice.Fields("N_DECLAR") & Sep &_
		dbfPrice.Fields("SERTIF") & Sep & dbfPrice.Fields("SERII") & Sep & Sep & dbfPrice.Fields("SROK") & Sep & Sep & Sep & Sep & Sep & dbfPrice.Fields("SUMMA") & Sep & Sep
