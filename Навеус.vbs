...
For Each FF in FL
 ...
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Sum(CENAO) AS Summ FROM ["&TableName&"]" 'запрос на расчет суммы по накладной
  St=TableName & Sep & DATE & Sep 
  ...
  dbfPrice.MoveNext
    ...
    St = dbfPrice.Fields("ARTIKUL") & Sep & dbfPrice.Fields("NAME") & Sep & dbfPrice.Fields("NAMEIZG") & Sep & dbfPrice.Fields("STRANAIZG") & Sep & dbfPrice.Fields("KOL") & Sep & dbfPrice.Fields("CENAO") & Sep & dbfPrice.Fields("CENAZ") & Sep & dbfPrice.Fields("CENAO")-(dbfPrice.Fields("CENAO")*dbfPrice.Fields("NDS")/(dbfPrice.Fields("NDS")+100)) & Sep & Sep & Sep & Sep & dbfPrice.Fields("GTD") & Sep & dbfPrice.Fields("NP") & "^" & dbfPrice.Fields("SERTIFIKAT")& "^" & dbfPrice.Fields("SERTORG") & "^" & dbfPrice.Fields("SERTDATA1") & "^" & dbfPrice.Fields("SERTDATA2") & Sep & Sep & Sep & dbfPrice.Fields("GODNOST") & Sep & Sep & Sep & Sep & Sep & dbfPrice.Fields("CENAO")*dbfPrice.Fields("KOL") & Sep
	  dbfPrice.MoveNext
