...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  St=dbfPrice.Fields("N_NAK") & Sep & dbfPrice.Fields("DATANAK") & Sep & dbfSum("Summ")
  St=Convert866to1251(St)
  DeviceOut.WriteLine St
  DeviceOut.WriteLine "[Body]"
      
    Do While Not dbfPrice.Eof
	  St = dbfPrice("KOD") & Sep & ReplaceStr(ReplaceStr(dbfPrice.Fields("NAME"),Chr(13),""),Chr(10)," ") & Sep & dbfPrice.Fields("PROIZV") & Sep & dbfPrice.Fields("COUNTRY") & Sep &_
  	    dbfPrice.Fields("KOLVO") & Sep & dbfPrice.Fields("CENASNDS") & Sep & dbfPrice.Fields("CENAPROIZ") & Sep & dbfPrice.Fields("CENABNDS") & Sep &_
		Sep & Sep & Sep & dbfPrice.Fields("N_DECLAR") & Sep &_
		ReplaceStr(dbfPrice.Fields("SERTIF"),";",",") & Sep &_
		dbfPrice.Fields("SERII") & Sep & Sep & dbfPrice.Fields("DATAEND") & Sep & Sep & Sep &_
		dbfPrice.Fields("REESTR") & Sep & Sep & dbfPrice.Fields("SUMMA") & Sep & ""
          St=Convert866to1251(St)
          DeviceOut.WriteLine(St)
	  dbfPrice.MoveNext
...
Function ReplaceStr(What,FromS,ToS)
  if IsNull(What)=True or What="" then
    ReplaceStr=""
  else
    ReplaceStr=Replace(What,FromS,ToS)
  end if
end Function
