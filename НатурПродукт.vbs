...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  DeviceOut.WriteLine dbfPrice.Fields("DOC_NUM") & Sep & dbfPrice.Fields("DOC_DATE") & Sep & dbfPrice.Fields("DOC_SUM_A")
  DeviceOut.WriteLine "[Body]"
  
    Do While Not dbfPrice.Eof
	  St = dbfPrice.Fields("PRODUCT_ID") & Sep & dbfPrice.Fields("PRODUCT_N") & Sep & dbfPrice.Fields("CREATOR") & Sep & dbfPrice.Fields("COUNTRY") & Sep & dbfPrice.Fields("COUNT") & Sep & dbfPrice.Fields("SUM_ADD")/dbfPrice.Fields("COUNT") & Sep & Sep & dbfPrice.Fields("SUM_ADD")/dbfPrice.Fields("COUNT")-dbfPrice.Fields("SUM_ADD")/dbfPrice.Fields("COUNT")/(dbfPrice.Fields("ADD_TAX_V")+100)*dbfPrice.Fields("ADD_TAX_V") & Sep &_
	       Sep & Sep & Sep &  Sep & dbfPrice.Fields("CERTIF")& "^" &  "^"  & Sep &_
	       Replace(dbfPrice.Fields("SERIES"),"-","") & Sep & Sep & dbfPrice.Fields("SERV_LIFE") & Sep & Sep & Sep &_ 
	       Sep & Sep & dbfPrice.Fields("SUM_ADD") & Sep & ""
