...
For Each FF in FL
 ...
 if InStr(LCase(FF.Name),"."&InExt) AND FF.Size<50000 then
  ...
  DeviceOut.WriteLine Convert866to1251(dbfPrice.Fields("NUM_NAK")) & Sep & dbfPrice.Fields("DATE_NAK") & Sep & dbfSum.Fields("SUMM")
  DeviceOut.WriteLine "[Body]"
  
    Do While Not dbfPrice.Eof
          if IsNull(dbfPrice.Fields("SERIA"))=True then
	    Exit do
	  end if  
	  Arr=Split(dbfPrice.Fields("SERIA"),"(")
          Seria=arr(0)
          St=Sep & dbfPrice.Fields("NAME_TOVAR") & Sep & dbfPrice.Fields("PROIZ") & Sep & dbfPrice.Fields("COUNTRY") & Sep &_
  	        dbfPrice.Fields("KOLICH") & Sep & dbfPrice.Fields("SUMMA")/dbfPrice.Fields("KOLICH") & Sep & Sep & dbfPrice.Fields("SUMMA")/dbfPrice.Fields("KOLICH")-dbfPrice.Fields("SUMMA")/dbfPrice.Fields("KOLICH")/(dbfPrice.Fields("NDS_STAVKA")+100)*dbfPrice.Fields("NDS_STAVKA") & Sep &_
		Sep & dbfPrice.Fields("NACENKA_PR") & Sep & Sep & dbfPrice.Fields("GTD") & Sep & Seria & "^" &_
		dbfPrice.Fields("SERTIFICAT")& "^" & "^" &""& Sep &_
		Seria & Sep & Sep & "01."&dbfPrice.Fields("GODEN_DO") & Sep & Sep & dbfPrice.Fields("DATA_REEST")& Sep &_
		Sep & Sep & dbfPrice.Fields("SUMMA") & Sep
	  dbfPrice.MoveNext
