...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfPrice.Open "SELECT * FROM " &TableName ' Файл
  ...
  DeviceOut.WriteLine "[Body]"
    ...
    Do While Not dbfPrice.Eof
	   St=dbfPrice("KOD") & Sep & dbfPrice.Fields("NAME") & Sep & dbfPrice.Fields("PROIZV") & Sep & dbfPrice.Fields("COUNTRY") & Sep &_
  	    dbfPrice.Fields("KOLVO") & Sep & dbfPrice.Fields("CENACNDS") & Sep & dbfPrice.Fields("CENABNDS") & Sep & dbfPrice.Fields("CENABNDS") & Sep &_
		Sep & Sep & Sep & dbfPrice.Fields("N_DECLAR") & Sep &_
		dbfPrice.Fields("SERTIF") & Sep &_
