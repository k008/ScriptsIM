...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Sum(SUM) AS Summ FROM ["&TableName&"]" 'запрос на расчет суммы по накладной
 St=dbfPrice.Fields("NOM_DOC") & Sep & dbfPrice.Fields("DATA_DOC") & Sep & dbfSum("Summ")
  ...
  DeviceOut.WriteLine "[Body]"
    ...
    Do While Not dbfPrice.Eof
      St=dbfPrice.Fields("KOD_TOV") & Sep & dbfPrice.Fields("NAIM") & Sep & Sep & dbfPrice.Fields("IZGOT") & Sep &_
	    dbfPrice.Fields("KOL") & Sep & dbfPrice.Fields("SUM")/dbfPrice.Fields("KOL") & Sep & Sep & dbfPrice.Fields("SUM")/dbfPrice.Fields("KOL")*100/(dbfPrice.Fields("NDS")+100) & Sep & Sep & Sep & Sep & dbfPrice.Fields("GTD") & Sep & dbfPrice.Fields("SERT") & "^" & dbfPrice.Fields("VYDAN") & "^" & dbfPrice.Fields("SROK_S") & Sep & Sep & Sep & Sep & Sep & Sep & Sep & Sep & dbfPrice.Fields("SUM") & Sep & ZV
