...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  LenFF=Len(FF)
  LenTB=Len(TableName)
  If Mid(TableName, 1, 3) = 000 Then
    TableName=Mid(TableName,4,LenTB)
...
'  msgbox(FF)
  ...
  St=TableName & Sep & dbfPrice.Fields("DOC_DATE") & Sep & dbfSum("Summ")
  St=Convert866to1251(St)
  DeviceOut.WriteLine(St)
  DeviceOut.WriteLine "[Body]"
  
    Do While Not dbfPrice.Eof
          St=dbfPrice.Fields("CODE") & Sep & dbfPrice.Fields("NAME") & Sep & Sep & dbfPrice.Fields("STRANA") & Sep &_
	    dbfPrice.Fields("KOL") & Sep & dbfPrice.Fields("PRICESNDS") & Sep & Sep & dbfPrice.Fields("PRICEBNDS") & Sep &_
		Sep & dbfPrice.Fields("PROC_NDS") & Sep & Sep & dbfPrice.Fields("GTD") & Sep & dbfPrice.Fields("SERT") & "^" & dbfPrice.Fields("SERT_FIRM") & "^" & dbfPrice.Fields("GOD_SERT") & Sep & dbfPrice.Fields("SERIA") & Sep & Sep & dbfPrice.Fields("GODENDO") & Sep &_
		dbfPrice.Fields("EAN13") & Sep & Sep & Sep & Sep & dbfPrice.Fields("SUMMA") & Sep
