...
For Each FF in FL
 if InStr(FF.Name,"."&InExt) then

  TableName=Mid(FF.Name,1,InStr(FF.Name,"."&InExt)-1)
  ...
  dbfSum.Open "SELECT Sum(S1) AS Summ FROM (SELECT KOL*COTP AS S1 FROM ["&TableName&"]) AS T" 'запрос на расчет суммы по накладной
  St=dbfPrice.Fields("Name") & Sep & dbfPrice.Fields("SROK") & Sep & dbfSum("Summ")
  ...
  DeviceOut.WriteLine "[Body]"
    ...
    Do While Not dbfPrice.Eof
      St=dbfPrice.Fields("KOD1") & Sep & dbfPrice.Fields("NAME") & Sep & dbfPrice.Fields("IZGOT") & Sep & dbfPrice.Fields("STRANA") & Sep &_
	    dbfPrice.Fields("KOL") & Sep & dbfPrice.Fields("COTP") & Sep & dbfPrice.Fields("CZI") & Sep & dbfPrice.Fields("CZI") & Sep &_
		Sep & "0" & Sep & Sep & dbfPrice.Fields("GTD") & Sep & dbfPrice.Fields("SERT") &_
		"^" & dbfPrice("DATE_SERT") & dbfPrice("ORG_SERT") & "^" & dbfPrice("SROK_SERT") &_
		Sep & dbfPrice.Fields("SER") & Sep & Sep & dbfPrice.Fields("SROK") & Sep &_
		Sep & Sep & Sep & Sep & dbfPrice.Fields("KOL")*dbfPrice.Fields("COTP") & Sep & dbfPrice.Fields("GV")
