...
Dim FSO,Sep,FDir,FLD,ArrayInp,FF,FL,St,i,ZV
...
For Each FF in FL
 ...
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  DeviceOut.WriteLine Trim(dbfRS.Fields("NOM_DOC")) & Sep & Trim(dbfRS.Fields("DATE_DOC")) & Sep & dbfSum.Fields("SUMMADOC1") 
  DeviceOut.WriteLine "[Body]" 
  
    Do While Not dbfRS.Eof

	
     if IsNull(dbfRS.Fields("PCT_NDS"))=True then
	    Exit do
     end if
     if dbfRS.Fields("ZV") = True then
	    ZV = 1
	  else
	    ZV = 0
	  end if
     ArrayOut(0)= Trim(dbfRS.Fields("CODE"))    ' Код товара
     ...
     ArrayOut(7)= CDbl(Trim(dbfRS.Fields("PRICE")))/(1+CDbl(Trim(dbfRS.Fields("PCT_NDS")))/100)     ' Цена зак. без НДС
     ArrayOut(8)=""    
     ArrayOut(9)= ""        ' Наценка посредника
     ArrayOut(10)=""        
     ArrayOut(11)= Trim(dbfRS.Fields("GTD"))   ' ГТД
     ArrayOut(12)=Trim(dbfRS.Fields("DOCUMENT"))&"^"&Trim(dbfRS.Fields("KAL"))&"^"&Trim(dbfRS.Fields("END_D")) 'сертификаты
