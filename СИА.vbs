...
Dim TableName,dbfPrice,dbfSum,dbfConn,ZV,ErrC, WSHShell,CName
...
'St="C:\mail\util\rar.exe e -y " & FDir & "*.rar" & " " & FDir
...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
   FSO.CopyFile FDir&FF.Name, FDir&CName & "." & InExt
...
For Each FF in FL
 ...
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Sum(SUMMA) AS Summ FROM ["&TableName&"]" 'запрос на расчет суммы по накладной
  St=dbfPrice.Fields("NUM_DOC") & Sep & dbfPrice.Fields("DATE_DOC") & Sep & dbfSum.Fields("Summ")
  St=Convert866to1251(St)
  DeviceOut.WriteLine St
  DeviceOut.WriteLine "[Body]"
  
    Do While Not dbfPrice.Eof
          St=dbfPrice.Fields("KOD")&Sep & dbfPrice.Fields("NAME") & Sep & dbfPrice.Fields("PROIZV") & Sep & dbfPrice.Fields("COUNTRY") & Sep &_
  	        dbfPrice.Fields("KOLVO") & Sep & dbfPrice.Fields("CENASNDS") & Sep & dbfPrice.Fields("CENAPROIZ") & Sep & dbfPrice.Fields("CENABNDS") & Sep &_
		Sep & dbfPrice.Fields("NADBPROC") & Sep & Sep & dbfPrice.Fields("N_DECLAR") & Sep &_
		dbfPrice.Fields("SERTIF")& "^" & dbfPrice.Fields("REG_NUM") & "^" & dbfPrice.Fields("DATAEND") & Sep &_
		dbfPrice.Fields("SERII") & Sep & Sep & dbfPrice.Fields("SROK") & Sep & dbfPrice.Fields("SCANCODE") & Sep & dbfPrice.Fields("DATAREGSE") & Sep &_
		dbfPrice.Fields("REESTR") & Sep & Sep & dbfPrice.Fields("SUMMA") & Sep & dbfPrice.Fields("JNVLN")
	  dbfPrice.MoveNext
