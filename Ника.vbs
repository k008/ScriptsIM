...
For Each FF in FL
 ...
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Sum(VSEGO) AS Summ FROM ["&TableName&"]" 'запрос на расчет суммы по накладной
  St=TableName & Sep & Date & Sep & dbfSum.Fields("Summ")
  ...
  DeviceOut.WriteLine "[Body]"
    ...
    Do While Not dbfPrice.Eof
		...
		dbfPrice.Fields("SERIYA") & Sep & Sep & dbfPrice.Fields("SROKGODN") & Sep & Sep & Sep &_
