...
Randomize
Const TemplateFile="X:\PRnakl\PRnakl.ots" '"X:\PRnakl\PRnakl.ots" ' Файл с шаблоном отчета
FDirOut="X:\PRnakl\tmp\"   ' Путь куда выкладывать файл с отчетом
FileOutXLS="общий1 склад " & Date & " " & Int(Rnd*999999) & ".xls"
FileIn=TemplateFile
...
If InStr(1,dbfPrice.Fields("NAMESUB2"), SKM) Then
	...
	FileOutXLS="Склад СК-МедФарм " & Month(Date) & "." & Year(Date) & ".xls"
...
Else
	...
	FileOutXLS="Общий Склад " & Month(Date) & "." & Year(Date) & ".xls"
	...
	If Not IsEmpty(EAN) Then
		...
		Do While Not dbfPrice.Eof
			...
			'	End If
				...
				Call oSheet.getCellByPosition(9, startcol+n).SetFormula("Соблюдены")
				Call oSheet.getCellByPosition(10, startcol+n).SetFormula("Соответствует")
				Call oSheet.getCellByPosition(11, startcol+n).SetFormula("Укомплектованно")
...
If Not IsEmpty(EAN) Then
	...
	' Ошибка если стоит Overwrite: False!!!
End If
