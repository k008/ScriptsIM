...
FDir="C:\Braki\2015\"      ' Путь, где смотреть Документы с сайта в формате Excel
FDirOut="C:\braki\0\"   ' Путь куда выкладывать файл с отчетом
TemplateFile="C:\braki\ReportLS.ots" ' Файл с шаблоном отчета
...
MonthYear=InputBox("Введите месяц. Пример: 01.12 - январь 2012г.","Введите месяц",Mid(CStr(Date),4,2)&"."&Mid(CStr(Date),9,2))
...
startcol=10	   
For Each FF in FL
  if (InStr(LCase(FF.Name),".xls") and (InStr(LCase(FF.Name),MonthYear))) then
    ...
    while xlWbk.getCellByPosition(7,k).String<>"Документы"
      k=k+1
    wend
    k=k+1
    'n=n+1
...
Call oSheet.getCellByPosition(1, startcol+n).SetFormula(Mid(Replace(xlWbk.getCellByPosition(7,k).String,C,"'"),1,200))
'Call oSheet.getCellByPosition(1, startcol+n).SetFormula(LetNum)
Call oSheet.getCellByPosition(2, startcol+n).SetFormula(Mid(Replace(xlWbk.getCellByPosition(1,k).String,C,"'"),1,200))
Call oSheet.getCellByPosition(3, startcol+n).SetFormula(Mid(Replace(xlWbk.getCellByPosition(5,k).String,C,"'"),1,200))
Call oSheet.getCellByPosition(4, startcol+n).SetFormula(Mid(Replace(xlWbk.getCellByPosition(2,k).String,C,"'"),1,200) & ", " & Mid(Replace(xlWbk.getCellByPosition(3,k).String,C,"'"),1,200))
...
Next
 ...
 MonthYear=MonthYear&" месяц "
 if Len(ar1(1))=2 then
   MonthYear=MonthYear&"20"
 end if
 MonthYear=MonthYear&ar1(1)&"г."
 Call oSheet.getCellByPosition(6, 6).SetFormula(MonthYear)
 '-------------------------------------------------------------
