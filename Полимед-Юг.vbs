...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Sum(KOL*PRICEOPS) AS Summ FROM ["&TableName&"]"  'запрос на расчет суммы по накладной
  ...
  DeviceOut.WriteLine "[Body]"
    ...
    Do While Not dbfPrice.Eof
      St=dbfPrice.Fields("KODPOST") & Sep & dbfPrice.Fields("NAMETOVAR") & Sep & dbfPrice.Fields("PROIZV") & Sep & Sep &_
	    dbfPrice.Fields("KOL") & Sep & dbfPrice.Fields("PRICEOPS") & Sep & dbfPrice.Fields("PRICEPRB") & Sep & dbfPrice.Fields("PRICEOPB") & Sep &_
		Sep & "0" & Sep & Sep & Sep &_
		dbfPrice.Fields("SERTIF") &	Sep & dbfPrice.Fields("SERIJA") & Sep & Sep & dbfPrice.Fields("GODEN") & Sep &_
		Sep & dbfPrice.Fields("DATEGOS") & Sep & dbfPrice.Fields("PRICEGOS") & Sep &_
		Sep & dbfPrice.Fields("KOL")*dbfPrice.Fields("PRICEOPS") & Sep & dbfPrice.Fields("GV")
