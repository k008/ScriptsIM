...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Sum(((PRICE_NAKE)+((PRICE_NAKE)/100*(NDS_PR)))*(QUANTITY)) AS Summ FROM ["&TableName&"]" 
  St=dbfPrice.Fields("N_NACL") & Sep & dbfPrice.Fields("D_NACL") & Sep & dbfSum("Summ")
  St=Convert866to1251(St)
  DeviceOut.WriteLine St
  DeviceOut.WriteLine "[Body]"
      
    Do While Not dbfPrice.Eof
    St=dbfPrice("CODE") & Sep & dbfPrice.Fields("NAME") & Sep & dbfPrice.Fields("FACTORY") & Sep & dbfPrice.Fields("COUNTRY") & Sep &dbfPrice.Fields("QUANTITY") & Sep & dbfPrice.Fields("NDS_SUM") & Sep & dbfPrice.Fields("PRICE_MAKE") & Sep & dbfPrice.Fields("PRICE_NAKE") & Sep & Sep & Sep & Sep & dbfPrice.Fields("GTD") & Sep & dbfPrice.Fields("SERT") & "^" & Sep & dbfPrice.Fields("SERIES") & Sep & Sep & dbfPrice.Fields("DATE_VALID") & Sep & dbfPrice.Fields("SCANCOD") & Sep & dbfPrice.Fields("DATE_REES") & Sep & dbfPrice.Fields("PRICE_REES") & Sep & Sep & (dbfPrice.Fields("PRICE_NAKE")+(dbfPrice.Fields("PRICE_NAKE")/100*(dbfPrice.Fields("NDS_PR"))))*(dbfPrice.Fields("QUANTITY")) & Sep & ""
          St=Convert866to1251(St)
          DeviceOut.WriteLine(St)
	  dbfPrice.MoveNext
