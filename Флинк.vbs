...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Sum(SYM) AS Summ FROM ["&TableName&"]"
  St=dbfPrice.Fields("NOMD") & Sep & dbfPrice.Fields("DATD") & Sep & dbfSum("Summ")
  St=Convert866to1251(St)
  DeviceOut.WriteLine St
  DeviceOut.WriteLine "[Body]"
      
    Do While Not dbfPrice.Eof
	  St = dbfPrice("ARTIKYL") & Sep & dbfPrice.Fields("TOV") & Sep & Sep & Sep &dbfPrice.Fields("KOL") & Sep & dbfPrice.Fields("CENA") & Sep & Sep & dbfPrice.Fields("CENA") & Sep & Sep & Sep & Sep & Sep & Sep & Sep & Sep & Sep & Sep & Sep & Sep & Sep & dbfPrice.Fields("SYM") & Sep & ""
          St=Convert866to1251(St)
          DeviceOut.WriteLine(St)
	  dbfPrice.MoveNext
