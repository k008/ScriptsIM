...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  'dbfSum.Open "SELECT Sum(CInt(Summa)) AS Summ FROM ["&TableName&"]" 'запрос на расчет суммы по накладной
  ...
  St=dbfPrice.Fields("KODTOV") & Sep & dbfPrice.Fields("NAMETOV") & Sep & dbfPrice.Fields("PROIZ") & Sep & dbfPrice.Fields("STRANA") & Sep & dbfPrice.Fields("KOL") & Sep & dbfPrice.Fields("CENA") & Sep & Sep & dbfPrice.Fields("CENABEZNDS") & Sep & Sep & "0" & Sep & Sep & Sep & dbfPrice.Fields("SERT") & Sep & Sep & Sep & Sep & Sep & dbfPrice.Fields("GODENDO") & Sep & Sep & Sep & dbfPrice.Fields("SUMMA") & Sep
