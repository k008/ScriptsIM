...
For Each FF in FL
 ...
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  TableName=Mid(FF.Name,1,InStr(LCase(FF.Name),"."&InExt)-1)
  ...
  dbfSum.Open "SELECT Sum(Summa) AS Summ FROM "&TableName ' Файл (расчет суммы)

  DeviceOut.WriteLine Convert866to1251(dbfPrice.Fields("NOM_DOC")) & Sep & dbfPrice.Fields("DATE_DOC") & Sep & dbfSum("Summ")
  DeviceOut.WriteLine "[Body]"
  
    Do While Not dbfPrice.Eof
	  St=DelZero(dbfPrice.Fields("CODE")) & Sep & dbfPrice.Fields("NAME_TOVAR") & Sep & dbfPrice.Fields("PROIZ") & Sep & dbfPrice.Fields("COUNTRY") & Sep &_
  	    dbfPrice.Fields("VOLUME") & Sep & dbfPrice.Fields("PRICE") & Sep & dbfPrice.Fields("NACENKA") & Sep & dbfPrice.Fields("PRICEWNDS") & Sep &_
		Sep & Sep & Sep & dbfPrice.Fields("GTD") & Sep & dbfPrice.Fields("SERIA") & "^" &_
		dbfPrice.Fields("DOCUMENT") & "^" & dbfPrice.Fields("BEGIN_D") & " " & dbfPrice.Fields("KAL") & "^" & dbfPrice.Fields("END_D") & Sep &_
		dbfPrice.Fields("SERIA") & Sep & Sep & dbfPrice.Fields("SROK") & Sep & Sep & Sep &_
		dbfPrice.Fields("GOS_R") & Sep & Sep & dbfPrice.Fields("SUMMA") & Sep & dbfPrice.Fields("JZN")
          dbfPrice.MoveNext
...
For i=1 to Len(St) 
 Stmp=Mid(St,i,1)
 if IsNumeric(Stmp) AND Stmp<>"0" then exit for
next 
