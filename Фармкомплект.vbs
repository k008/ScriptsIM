...
For Each FF in FL
 ...
 if InStr(LCase(FF.Name),"."&InExt) AND FF.Size<50000 then
  ...
  DeviceOut.WriteLine Convert866to1251(dbfPrice.Fields("NDOC") & Sep & dbfPrice.Fields("DATEDOC") & Sep & dbfSum.Fields("Summ"))
  DeviceOut.WriteLine "[Body]"
    Do While Not dbfPrice.Eof
	  if Trim(dbfPrice.Fields("JNVLS"))="Ж" then
        ZV = 1
      else
        ZV = 0
      end if
          St=dbfPrice.Fields("CODENAME") & Sep & dbfPrice.Fields("NAME") & Sep & dbfPrice.Fields("MANUF") & Sep & dbfPrice.Fields("COUNTRY") & Sep &_
  	        dbfPrice.Fields("QNT") & Sep & dbfPrice.Fields("PRICENDS") & Sep & dbfPrice.Fields("PRICEPRO") & Sep & dbfPrice.Fields("PRICE") & Sep &_
		Sep & Sep & Sep & dbfPrice.Fields("GTD") & Sep &_
		dbfPrice.Fields("SERTIF")& "^" & dbfPrice.Fields("CENTRESERT") & "^" & dbfPrice.Fields("DATESERT") & "^" & dbfPrice.Fields("SROKSERT") & Sep &_
		dbfPrice.Fields("SERIA") & Sep & Sep & dbfPrice.Fields("SROKSERIA") & Sep & dbfPrice.Fields("ZSHK") & Sep & Sep & dbfPrice.Fields("REG") &_
		Sep & Sep & dbfPrice.Fields("SUMMA") & Sep & ZV
	  dbfPrice.MoveNext
