...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Sum((PRICE)*(KOLVO)) AS Summ FROM ["&TableName&"]"
  ...
  DeviceOut.WriteLine "[Body]"
    ...
    Do While Not dbfPrice.Eof
	  St = dbfPrice("KOD") & Sep & dbfPrice.Fields("NAME_TOVAR") & Sep & dbfPrice.Fields("PROIZ") & Sep & dbfPrice.Fields("COUNTRY") & Sep &dbfPrice.Fields("KOLVO") & Sep & dbfPrice.Fields("PRICE") & Sep & Sep & dbfPrice.Fields("PRICE_WO") & Sep & Sep & Sep & Sep & Sep & ReplaceStr(dbfPrice.Fields("SERT"),Sep,",") & Sep & dbfPrice.Fields("SERIA") & Sep & Sep & dbfPrice.Fields("SROK") & Sep & Sep & Sep & Sep & Sep & ((dbfPrice.Fields("KOLVO"))*(dbfPrice.Fields("PRICE"))) & Sep & ""
          St=Convert866to1251(St)
          DeviceOut.WriteLine(St)
	  dbfPrice.MoveNext
