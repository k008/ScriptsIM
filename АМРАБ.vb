...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
   FSO.CopyFile FDir&FF.Name, FDir&Mid(FF.Name,InStr(FF.Name,")")+1,InStr(FF.Name,"-")-InStr(FF.Name,")")-1) & "." & InExt
...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  TableName=UCase(Mid(LCase(FF.Name),1,InStr(LCase(FF.Name),"."&InExt)-1))
  ...
  dbfSum.Open "SELECT Sum(S1) AS Summ FROM (SELECT KOLF*OTP AS S1 FROM ["&TableName&"]) AS T" 'запрос на расчет суммы по накладной
  ...
  DeviceOut.WriteLine "[Body]"
    ...
    Do While Not dbfPrice.Eof
      St=dbfPrice.Fields("NNMT") & Sep & dbfPrice.Fields("NAIM") & Sep & dbfPrice.Fields("PROIZ") & Sep & Sep &_
	    dbfPrice.Fields("KOLF") & Sep & dbfPrice.Fields("OTP") & Sep & dbfPrice.Fields("COPT") & Sep & dbfPrice.Fields("COPT") & Sep &_
		Sep & dbfPrice.Fields("NACT") & Sep & Sep & dbfPrice.Fields("GTD") & Sep & Replace(dbfPrice.Fields("SERIA"),";",",") &_
		Sep & Mid(dbfPrice.Fields("SERIA"),1,InStr(dbfPrice.Fields("SERIA"),"(")-2) & Sep & Sep & dbfPrice.Fields("NED2") & Sep &_
		Sep & Sep & Sep & Sep & dbfPrice.Fields("KOLF")*dbfPrice.Fields("OTP") & Sep
