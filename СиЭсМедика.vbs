...
Dim TableName,dbfPrice,dbfConn
...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  St=dbfPrice.Fields("NOMER") & Sep & dbfPrice.Fields("DATA") & Sep & dbfPrice("SUMM_N_TOT")
  St=Convert866to1251(St)
  DeviceOut.WriteLine(St)
  DeviceOut.WriteLine "[Body]"
  
    Do While Not dbfPrice.Eof
      St=dbfPrice.Fields("KOD_TOV") & Sep & dbfPrice.Fields("NAME_TOV") & Sep & Sep & dbfPrice.Fields("COUNTRY") & Sep &_
	    dbfPrice.Fields("KOL") & Sep & dbfPrice.Fields("TOTAL")/dbfPrice.Fields("KOL") & Sep & Sep & dbfPrice.Fields("TOTAL")/dbfPrice.Fields("KOL")-dbfPrice.Fields("TOTAL")/dbfPrice.Fields("KOL")/(dbfPrice.Fields("P_STNDS")+100)*dbfPrice.Fields("P_STNDS") & Sep &_
		Sep & "0" & Sep & Sep & dbfPrice.Fields("GTD") & Sep & dbfPrice.Fields("SERTIF") &_
		Sep & Sep & Sep & Sep &	Sep & Sep & Sep & Sep & dbfPrice.Fields("TOTAL") & Sep
