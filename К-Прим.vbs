...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Sum(SUMPAY) AS Summ FROM "&TableName 'запрос на расчет суммы по накладно
  ...
  DeviceOut.WriteLine "[Body]"
    ...
    Do While Not dbfPrice.Eof
      St=dbfPrice.Fields("CODEPST") & Sep & dbfPrice.Fields("NAME") & Sep & dbfPrice.Fields("FIRM") & Sep & dbfPrice.Fields("CNTR") & Sep &_
	    dbfPrice.Fields("QNT") & Sep & dbfPrice.Fields("PRICE1") & Sep & Sep & dbfPrice.Fields("PRICE2N") & Sep &_
		Sep & dbfPrice.Fields("PRCOPT") & Sep & Sep & dbfPrice.Fields("NUMGTD") & Sep &_
		dbfPrice.Fields("SERTIF") & "^" & dbfPrice.Fields("SERTORG") &_
		Sep & dbfPrice.Fields("SER") & Sep & Sep & dbfPrice.Fields("GDATE") & Sep &_
		Sep & dbfPrice.Fields("DATEPRC") & Sep & dbfPrice.Fields("REGPRC") & Sep &_
		Sep & dbfPrice.Fields("SUMPAY") & Sep & dbfPrice.Fields("GNVLS")
