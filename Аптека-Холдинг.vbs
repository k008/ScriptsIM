...
Dim DeviceInp,DeviceOut,Counter,TableName1
...
'   TableName1=Mid(LCase(FF.Name),1,InStr(LCase(FF.Name),"."&InExt)-1)
   CheckLenTableName()
   'TableName1=Mid(TableName1, 1, Instr(1,TableName1,"-")-1) & Mid(TableName1, Instr(1,TableName,"-")+1, LenTB-1)
'    msgbox(tablename1)
...
'For Each FF in FL 'Смотрим все файлы и убираем год
 'if InStr(LCase(FF.Name),"."&InExt) then
   'FSO.CopyFile FDir&FF.Name, FDir&CInt(Mid(FF.Name,1,InStr(FF.Name,".")-1))&"."&InExt
...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Sum(SUMMANDS) AS Summ FROM ["&TableName&"]" 'запрос на расчет суммы по накладной
  St=dbfPrice.Fields("DOCNO") & Sep & dbfPrice.Fields("DOCDAT") & Sep & dbfSum("Summ")
  St=Convert866to1251(St)
  DeviceOut.WriteLine(St)
  DeviceOut.WriteLine "[Body]"
  
    Do While Not dbfPrice.Eof
	  if Trim(dbfPrice.Fields("NG"))="Ж" then
	    ZV = 1
	  else
	    ZV = 0
	  end if	
          St=dbfPrice.Fields("CODE") & Sep & dbfPrice.Fields("TOVAR") & Sep & dbfPrice.Fields("PROIZV") & Sep & dbfPrice.Fields("STRANA") & Sep &_
	    dbfPrice.Fields("KOL") & Sep & dbfPrice.Fields("TZENANDS") & Sep & dbfPrice.Fields("ZAVOD") & Sep & dbfPrice.Fields("TZENA") & Sep &_
		Sep & "0" & Sep & Sep & dbfPrice.Fields("GTD") & Sep &_
		dbfPrice.Fields("SERTIF") & "^" & dbfPrice.Fields("SERTOT") & " " & "^" & dbfPrice.Fields("SERTDO") &_
		Sep & dbfPrice.Fields("SERIA") & Sep & Sep & dbfPrice.Fields("GODEN") & Sep & dbfPrice.Fields("SKOD") &_
		Sep & dbfPrice.Fields("DATREG") & Sep & dbfPrice.Fields("REG") & Sep &_
		Sep & dbfPrice.Fields("SUMMANDS") & Sep & ZV
  ...
  For Each FF in FL
    If InStr(LCase(FF.Name),"."&InExt) Then
      ...
      If Len(TableName1)>8 Then 
        ...
        FSO.DeleteFile FDir&FF.Name
        If Len(DBN)>8 Then
          'msgbox ("Необходимо уменьшить имя накладной до 8-ми символов перед '.dbg'. Количество символов=" & Len(DBN) & Chr(13)&Chr(10) & "Будет крах, звонить 911 с корпоративного телефона")
        End If
      End If
    End If
  Next
