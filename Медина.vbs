...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Sum(ZENAOTP*KOLICH) AS Summ FROM ["&TableName&"]" 'запрос на расчет суммы по накладной
  St=dbfPrice.Fields("TOVAR") & Sep & dbfPrice.Fields("DATEGODN") & Sep & dbfSum("Summ")
  ...
  dbfPrice.MoveNext
    ...
    Do While Not dbfPrice.Eof
      St=dbfPrice.Fields("KOD") & Sep & dbfPrice.Fields("TOVAR") & Sep & dbfPrice.Fields("IZGOT") & Sep & dbfPrice.Fields("STRIZGOT") & Sep &_
	    dbfPrice.Fields("KOLICH") & Sep & dbfPrice.Fields("ZENAOTP") & Sep & dbfPrice.Fields("ZENAZ") & Sep & dbfPrice.Fields("ZENABEZNDC") & Sep &_
		Sep & "0" & Sep & Sep & dbfPrice.Fields("GTD") & Sep &_
		dbfPrice.Fields("SERTIF") & "^" & dbfPrice.Fields("REGIST") &_
		Sep & dbfPrice.Fields("SERIA") & Sep & Sep & dbfPrice.Fields("DATEGODN") & Sep &_
		Sep & Sep & Sep & Sep & dbfPrice.Fields("ZENAOTP")*dbfPrice.Fields("KOLICH") & Sep
