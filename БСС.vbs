...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Sum(SUM_OLL) AS Summ FROM "&TableName
  St=dbfPrice.Fields("DOK_NAM") & Sep & dbfPrice.Fields("NAKLDATA") & Sep & dbfSum.Fields("Summ")   'ПОЧЕМУ-ТО РУГАЕТСЯ ПОЛЯ, НО ВРОДЕ ВСЁ ПРАВИЛЬНО
  'St=Sep & Sep & Sep
  ...
  DeviceOut.WriteLine "[Body]"
    ...
    Do While Not dbfPrice.Eof
	  St=dbfPrice.Fields("KOD") & Sep & dbfPrice.Fields("NAME") & Sep & dbfPrice.Fields("FIRM") & Sep & dbfPrice.Fields("STRANA") & Sep & dbfPrice.Fields("KOL") & Sep & dbfPrice.Fields("PRICE") & Sep &dbfPrice.Fields("PRICE_M2") & Sep & dbfPrice.Fields("PRICE_1") & Sep & Sep & Sep & Sep & dbfPrice.Fields("GTD") & Sep & dbfPrice.Fields("SERT_VS") & "^" & dbfPrice.Fields("GOD_SERT") & Sep & dbfPrice.Fields("SER") & Sep & Sep & dbfPrice.Fields("GOD_TO") & Sep & dbfPrice.Fields("EAN13") & Sep & Sep & dbfPrice.Fields("GRBNDS") & Sep & Sep & dbfPrice.Fields("SUM_OLL") & Sep & dbfPrice.Fields("GVLS") & Sep
          St=Convert866to1251(St)
          DeviceOut.WriteLine(St)
	  dbfPrice.MoveNext
