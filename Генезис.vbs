...
'St="C:\mail\util\rar.exe e -y " & FDir & "*.zip" & " " & FDir
St="C:\mail\util\pkzip25.exe -extr=update " & FDir & "*.zip" & " " & FDir
Set WSHShell = WScript.CreateObject("WScript.Shell")
ErrC=WSHShell.Run (St, 1 , True)
...
For Each FF in FL
 if InStr(LCase(FF.Name),"."&InExt) then
  ...
  dbfSum.Open "SELECT Sum(FULL_AMNT) AS Summ FROM "&TableName 'запрос на расчет суммы по накладной
  St=dbfPrice.Fields("TRX_NUM") & Sep & dbfPrice.Fields("TRX_DATE") & Sep & dbfSum("Summ")
  'St=dbfPrice.Fields("TRX_NUM") & Sep & dbfPrice.Fields("TRX_DATE") & Sep & dbfPrice.Fields("TAX_AMOUNT")
...
St=dbfPrice.Fields("ITEM_ID") & Sep & dbfPrice.Fields("ITEM_NAME") & Sep & dbfPrice.Fields("VEND_NAME") & Sep & Sep & dbfPrice.Fields("QNTY") & Sep & dbfPrice.Fields("FULL_AMNT")/dbfPrice.Fields("QNTY") & Sep & Sep &dbfPrice.Fields("FULL_AMNT")/dbfPrice.Fields("QNTY")-dbfPrice.Fields("FULL_AMNT")/dbfPrice.Fields("QNTY")*dbfPrice.Fields("TAX_RATE")/(dbfPrice.Fields("TAX_RATE")+100) & Sep & "0" & Sep & Sep & Sep & dbfPrice.Fields("DECL_NUM") & Sep & dbfPrice.Fields("REG_NUMBER") & "^" & dbfPrice.Fields("REG_CDATE") & " " & dbfPrice.Fields("CER_NUMBER") & "^" & dbfPrice.Fields("EXP_CDATE") & Sep & dbfPrice.Fields("LOT_NUMBER") & Sep & Sep & dbfPrice.Fields("EXP_DATE") & Sep & Sep & Sep & Sep & Sep & dbfPrice.Fields("FULL_AMNT") & Sep & ZV
